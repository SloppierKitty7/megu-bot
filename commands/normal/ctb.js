...
module.exports = {
	desc: "Display ctb stats for a user",
	usage: "<text>",
	hidden: false,
	ownerOnly: false,
	task(bot, msg, suffix) {
		var osuApi = new osu.api({
		  apiKey: config.osuapi
				...
				osuApi.getUser(osuApi.user.byUsername(suffix), osuApi.mode.ctb, function(err, response) {
				  if (err) {
				...
				function getBest () {
		  osuApi.getUserBest(osuApi.user.byUsername(suffix), osuApi.mode.ctb, function(err, response) { // PLEASE be mindful that osu.mode.default can be used as .all as it just = null. Default looks better for convenience here as this api call does NOT get all modes, default is standard.
			if (err) {
			  return console.log (err);
			}
